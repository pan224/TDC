...
module dynamic_phase_pulse_gen #(
    ...
    parameter PULSE_WIDTH = 2        // 脉冲宽度（时钟周期数），2 = 10ns @ 200MHz
)(
    input wire          clk_ref,        // 200MHz 参考时钟
    input wire          reset,
...
clk_wiz_phase clk_wiz_inst (
    .clk_in1(clk_ref),           // 200MHz 输入
    .clk_out1(clk_200MHz_shifted), // 200MHz 输出 (可调相位)
    .reset(reset),
...
// 同步 trigger 到 clk_200MHz_shifted 域
